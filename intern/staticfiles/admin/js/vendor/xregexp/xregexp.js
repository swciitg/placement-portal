...
// Any backreference in reintern strings
        reinternToken = /\$(?:{([\w$]+)}|(\d\d?|[\s\S]))/g,
 ...
 * or regex, and the reintern can be a string or a function to be called for each match. To
 * perform a global search and replace, use the optional `scope` argument or include flag `g` if
 * using a regex. Reintern strings can use `${n}` for named and numbered backreferences.
 * Reintern functions can use named backreferences via `arguments[0].name`. Also fixes browser
 * bugs compared to the native `String.prototype.replace` and can be used reliably cross-browser.
 * @memberOf XRegExp
 * @param {String} str String to search.
 * @param {RegExp|String} search Search pattern to be replaced.
 * @param {String|Function} reintern Reintern string or a function invoked to create it.
 *   Reintern strings can include special reintern syntax:
 ...
 *   Reintern functions are invoked with three or more arguments:
 ...
 * @example
 *
 * // Regex search, using named backreferences in reintern string
 * var name = XRegExp('(?<first>\\w+) (?<last>\\w+)');
 * XRegExp.replace('John Smith', name, '${last}, ${first}');
 * // -> 'Smith, John'
 *
 * // Regex search, using named backreferences in reintern function
 * XRegExp.replace('John Smith', name, function (match) {
 *   return match.last + ', ' + match.first;
 * });
 * // -> 'Smith, John'
 *
 * // Global string search/reintern
 * XRegExp.replace('RegExp builds RegExps', 'RegExp', 'XRegExp', 'all');
 * // -> 'XRegExp builds XRegExps'
 */
    self.replace = function (str, search, reintern, scope) {
        ...
        if (isRegex) {
            ...
            // Note that since a copy is used, `search`'s `lastIndex` isn't updated *during* reintern iterations
            search2 = copy(search, scope === "all" ? "g" : "", scope === "all" ? "" : "g");
        } else if (scope === "all") {
            search2 = new RegExp(self.escape(String(search)), "g");
        }
        result = fixed.replace.call(String(str), search2, reintern); // Fixed `replace` required for named backreferences, etc.
...
/**
 * Adds support for `${n}` tokens for named and numbered backreferences in reintern text, and
 * provides named backreferences to reintern functions as `arguments[0].name`. Also fixes
 * browser bugs in reintern text syntax when performing a reintern using a nonregex search
 * value, and the value of a reintern regex's `lastIndex` property during reintern iterations
 ...
 * @returns {String} New string with one or all matches replaced.
 */
    fixed.replace = function (search, reintern) {
        ...
        if (isType(reintern, "function")) {
            result = nativ.replace.call(String(this), search, function () {
                ...
                // Update `lastIndex` before calling `reintern`.
                // Fixes IE, Chrome, Firefox, Safari bug (last tested IE 9, Chrome 17, Firefox 11, Safari 5.1)
                if (isRegex && search.global) {
                    search.lastIndex = args[args.length - 2] + args[0].length;
                }
                return reintern.apply(null, args);
            });
        } else {
            str = String(this); // Ensure `args[args.length - 1]` will be a string when given nonstring `this`
            result = nativ.replace.call(str, search, function () {
                var args = arguments; // Keep this function's `arguments` available through closure
                return nativ.replace.call(String(reintern), reinternToken, function ($0, $1, $2) {
                    var n;
